...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    np.save('./char_attention/word.npy',final_embeddings)
    np.save('./char_attention/char.npy',final_char_embedding)
    saver.save(session, 'just_charattn.ckpt')
    ...
    if step % 10000 == 0:
      ...
      with open("./char_attention/tweet_list_%d.txt"%(count),mode="w") as fw:
