...
char_batch_size = 64
query_tokens = map(lambda x: dictionary[x],['avail','distribut','send'])
tweet_batch_size = 50
lambda_1 = 0.7
# word_max_len
# char_max_lens
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    if step % 10000 == 0:
      ...
      with open("./avail_char2vec/tweet_list_%d.txt"%(count),mode="w") as fw:
        fw.write('\n'.join(map(lambda x: str(x),file_list)))

  final_embeddings = normalized_embeddings.eval()
  final_char_embedding = normalized_char_embeddings.eval()
  np.save('./avail_char2vec/word.npy',final_embeddings)
  np.save('./avail_char2vec/char.npy',final_char_embedding)
  saver.save(session, "avail_char2vec.ckpt") 
