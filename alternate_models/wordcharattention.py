...
with graph.as_default():
  ...
  bilstm = tf.contrib.rnn.BidirectionalGridLSTMCell(num_units=128,couple_input_forget_gates=True)
  state = bilstm.zero_state()
  print(state.get_shape())
  for t in range(char_max_len):
    output, state = bilstm(character_word_embeddings[:,t],state=state)
